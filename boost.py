...
def loadDataSet(filename):
    ...
    labelMat = []
    fr = open(filename)
    for line in fr.readlines(): #readline() 和 readlines() 之间的差异是后者一次读取整个文件
        lineArr = []
        curLine = line.strip().split('\t')
        for i in range(numFeat-1):
            lineArr.append(float(curLine[i]))
        dataMat.append(lineArr)
        labelMat.append(float(curLine[-1])) #最后一列
...
def plotROC(preStrengths,classLabels):
    ...
    sortedIndicies = preStrengths.argsort()  #preStrengths为行向量组成的矩阵，代表分类器预测强度
    fig = plt.figure()
    fig.clf()
    ax = plt.subplot(111)
